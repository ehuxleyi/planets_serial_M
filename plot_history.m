...
h4.Position = [0.48 0.13 0.45 0.80];
h4exists = 1;
...
hold on;
plot(-8000, 12000, 'ro', 'LineWidth', 2, 'MarkerSize', 8);
hold on;
plot(-6000, 12000, '^r', 'MarkerSize', 10);
hold on;
...
% impact
for kk = 1:njumps2
    symsize = round(5.0*abs(jumps2(kk,2)/5.0)); % symbol size shows P size
...
% make the axes and tickmarks visible
set(gca, 'Layer', 'top');
uistack(ax,'top'); 
...
% bring the temperature history line to the top
uistack(li,'top');

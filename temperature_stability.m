...
% Matlab R2015a
%
% This version of the Planets model is the serial version (MATLAB)
%
...
for ii = 1:nplanets
    ...
    for jj = 1:nreruns
        ...
        while (((round(t(end))) < max_duration) && (y(end,1) > Tmin) && ...
            ...
            [t,y,te,ye,ie] = ode23s(@planets_ODE, [tstart tfinal], y0, ...
                options);
        ...
        % add the plot of this rerun to combined figure then pause for 4 s
        addto_rerunsfig;  pause(2);
        
    end  % of all reruns of the same planet
    
    % Create pushbutton to display all reruns on one figure
    btn2 = uicontrol('Style', 'pushbutton', 'String', 'Show Reruns',...
        'Position', [1200 80 100 30], 'Callback', 'showbuttonaction');
    % pause to give the user a chance to select the button, then delete it
    pause(6); removeshowbutton;
